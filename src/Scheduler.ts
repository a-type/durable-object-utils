...
import { DurableObjectStorage } from './cloudflareTypes.js';
...
export class Scheduler<Tasks extends { type: string; data?: any }> {
	constructor(
		...
		this.storage.getAlarm().then((alarm: number) => {
			if (alarm) {
				if (alarm < Date.now()) {
					...
					setTimeout(async () => {
						...
						if (alarm && alarm < Date.now()) {
							...
							console.warn(
								`Scheduler alarm is set in the past (${new Date(
									alarm,
								).toString()}); resetting to now.`,
							);
				...
				console.log(
					`Scheduler startup: current alarm is set for ${new Date(
						alarm,
					).toString()}`,
				);
			}
	...
	handleAlarm = async () => {
		...
		if (taskErrors.length > 0) {
			...
			throw new Error(
				`Errors processing scheduled tasks: ${taskErrors
					.map((e) => e.message)
					.join(', ')}`,
			);
		}
